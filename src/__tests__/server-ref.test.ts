 ...
 * `await` gaps inside async tool handlers (e.g. after awaiting a Kaseya
 * VSA API call, before sending an elicitation prompt back through "the"
 * server).
 *
...
describe("server-ref cross-tenant isolation", () => {
  ...
  it(
    ...
    async () => {
      ...
      // (simulating an in-flight Kaseya VSA API call inside a tool
      // handler) BEFORE sending its confirmation prompt.
